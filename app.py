...
Analyzes Android APK files to detect Google Play Integrity API usage
and Google Play Auto Protection (pairip) for sideloading risk assessment.
"""
...
st.caption(
    "Detect Google Play Integrity API and Auto Protection (pairip) in Android APKs "
    "to assess sideloading risk."
)
...
def _render_result(result: AnalysisResult):
    ...
    layers = [
        result.auto_protection,
        result.play_integrity,
        result.firebase_appcheck,
        result.safetynet,
        result.play_asset_delivery,
    ...
    if result.all_findings:
        table_data = [
            {
                ...
                "Pattern Matched": f.pattern_matched,
                "Detail": f.detail,
            }
        ...
        # CSV export
        csv_buf = io.StringIO()
        writer = csv.DictWriter(csv_buf, fieldnames=["Category", "Source File", "Pattern Matched", "Detail"])
        writer.writeheader()
        writer.writerows(table_data)
        st.download_button(
            "Download findings as CSV",
            csv_buf.getvalue(),
            file_name=f"{result.file_name}_findings.csv",
            mime="text/csv",
        )
    else:
...
with tab_batch:
    ...
    if "batch_results" in st.session_state:
        ...
        # CSV export for batch
        csv_buf = io.StringIO()
        writer = csv.DictWriter(csv_buf, fieldnames=[
            "File", "Package", "Size (MB)", "Risk Level", "Verdict", "Findings",
        ])
        writer.writeheader()
        writer.writerows(summary_data)
        st.download_button(
            "Download batch summary as CSV",
            csv_buf.getvalue(),
            file_name="batch_analysis_summary.csv",
            mime="text/csv",
        )

        # Expandable details per APK
